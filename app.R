...
ui<-shinyUI(
    dashboardPage(
        ...
        dashboardSidebar(
            sliderInput("bins","Number Of Breaks",1,1000,50),
            fluidPage(
              submitButton("Submit"),
            ),
            sidebarMenu(
...
server <- shinyServer(function(input, output){
    ...
    output$cloudtrump <- renderPlot({
        wordcloud(TrumpCorpus, min.freq = 30, max.words = 50)
    })
    output$cloudclinton <- renderPlot({
        wordcloud(ClintonCorpus, min.freq = 30, max.words = 50)
    })
    ...
    output$analisiss <- renderTable({
      head(Nrc %>% select(candidate, datetimestamp, id, language, word, sentiment), n=input$bins)
    })
